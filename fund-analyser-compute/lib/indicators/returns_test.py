...
def test_empty_returns_nan(lookback: str):
    assert np.isnan(MinReturns(lookback).calc(pd.Series()).value)
    assert np.isnan(MaxReturns(lookback).calc(pd.Series()).value)
...
def test_returns(lookback: str, expected_max: float, expected_min: float):
    actual_min = MinReturns(lookback).calc(SAMPLE_HISTORIC_PRICES)
    if np.isnan(expected_min):
        assert np.isnan(actual_min.value)
    else:
        assert actual_min.value == pytest.approx(expected_min, abs=0.01)

    actual_max = MaxReturns(lookback).calc(SAMPLE_HISTORIC_PRICES)
