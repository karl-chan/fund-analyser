...
def test_ppo_empty_returns_nan():
    assert np.isnan(PPO().calc(historic_prices=pd.Series()).value)


def test_ppo_short_returns_nan():
    assert np.isnan(PPO().calc(historic_prices=SAMPLE_HISTORIC_PRICES[:10]).value)


def test_ppo():
    actual = PPO().calc(historic_prices=SAMPLE_HISTORIC_PRICES)
