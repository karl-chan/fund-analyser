...
upper_band, middle_band, lower_band = bollinger_bands(merged_historic_prices)
...
def simulate_run(start_date: datetime):
    ...
    def restrict_isins(prices_df: pd.DataFrame, dt: datetime) -> List[str]:
        ...
        def avoid_downtrend(prices_df: pd.DataFrame, dt: datetime) -> List[str]:
            ...
            combined = pd.concat([last_gradient], axis=1)
        ...
        def avoid_negatives(prices_df: pd.DataFrame, dt: datetime) -> List[str]:
        ...
        def avoid_bollinger_top(prices_df: pd.DataFrame, dt: datetime) -> List[str]:
            ...
            # if dt.year == 2015 and dt.month == 8 and dt.day == 10:
            #     pdb.set_trace()
            return isins
...
if __name__ == "__main__":
    results = []
    # for run_begin_date in [datetime(2013, 1, 4)]:
    for run_begin_date in pd.date_range(start_date, start_date + hold_interval, freq='B'):
        ...
        results.append((run_begin_date, account, returns, drawdown))

    sorted_results = sorted(results, key=lambda tup: tup[2])
    min_begin_date, min_account, min_return, _ = sorted_results[0]
    max_begin_date, max_account, max_return, _ = sorted_results[-1]
    ...
    max_drawdown_date, _, _, max_drawdown = sorted_drawdowns[-1]
