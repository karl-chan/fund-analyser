...
function CharlesStanleyDirectAuth () {
    ...
    this.myDashboardUrl = 'https://www.charles-stanley-direct.co.uk/My_Dashboard'
...
CharlesStanleyDirectAuth.prototype.login = async function (user, pass, memorableWord) {
    ...
    await checkMemorableWord.apply(this, [ memorableWord, {csrfToken, jar} ])

    // Step 3: Account login checks
    await accountLoginChecks.apply(this, [ {jar} ])

    // Step 4: Get user name
    const {name} = await getMyAccount.apply(this, [ {jar} ])
...
const checkMemorableWord = async function (memorableWord, {csrfToken, jar}) {
...
const getMyAccount = async function ({jar}) {
