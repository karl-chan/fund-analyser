...
FundDAO.upsertFund = function (fund, callback) {
    ...
    db.get().collection('funds').updateOne(query, doc, {upsert: true}, (err, response) => {
        if (err) {
            return callback(err);
        }
...
FundDAO.listFunds = function (options, callback) {
    let query = buildQuery(options);
    query.toArray((err, docs) => {
        return callback(err, _.map(docs, FundDAO.toFund));
    });
};
...
FundDAO.exportCsv = function (savePath, options, headerFields, callback) {
    FundDAO.listFunds(options, (err, funds) => {
        ...
        const csvFile = json2csv({
            data: funds,
            fields: csv.mapFields(headerFields)
        });
