...
function computeAndAppendReturns(returns, historicPrices, lookbacks) {
    ...
    const historicPricesWithoutLast = _.cloneDeep(historicPrices);
    const latest = historicPricesWithoutLast.pop();
    ...
    _.forEach(lookbacks, (lookback) => {
        ...
        if (_.isEmpty(historicPricesWithoutLast) || beginDate.isBefore(historicPricesWithoutLast[0].date)) {
            return;
        }

        const beginRecord = _.minBy(historicPricesWithoutLast, (record) => {
