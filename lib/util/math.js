...
function closestRecord(lookback, historicPrices) {
    ...
    const latestDate = moment(_.last(historicPrices).date);
...
function calcPercentiles(returns, historicPrices, additionalLookbacks) {
    ...
    _.forEach(lookbacks, (lookback) => {
        ...
        const interestedRecords = _.dropWhile(historicPrices, (record) => moment(record.date).isBefore(beginRecord.date));
